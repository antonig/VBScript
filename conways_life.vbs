'Conway's Game of Life  32x32
'Uses an optimization found in Abrash' Black Book
'Setup your console to 34 rows minimum
...
sub paintframe(r,c,h,w)
  dim i,s
  wscript.StdOut.Write chr(27) &"(0"
  toxy r,c,"l"&string(w,"q")&"k"
  s="x"&space(w)&"x"
  for i=r+1 to h+1
    toxy i,c,s
  next
  toxy r+h+2,c,"m"&string(w,"q")&"j"
  wscript.StdOut.Write chr(27) &"(B"
end sub

function nextgen()
  dim r,c,pre,cnt,a1,onoff,chg
  nextgen=0
  a1=a                                 'does a hard a coopy of a
  for r=0 to maxdim
    for c=0 to maxdim
